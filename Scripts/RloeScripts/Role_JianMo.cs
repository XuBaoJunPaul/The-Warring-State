...
public class Role_JianMo :Role_Main  {
	...
	void Start(){
		skillPrefab = Resources.Load <GameObject>("TeXiao/JianMoSkill");
	...
	{
		GameObject skill = Instantiate (skillPrefab, skillFirePos.position, skillFirePos.rotation) as GameObject;
		if (skill != null) {
			JianMoSkillControl tmp = skill.GetComponent<JianMoSkillControl> ();
			tmp.camp = this.roleCamp;
			tmp.owner = this;
		}
