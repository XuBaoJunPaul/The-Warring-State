...
public class Role_YangJian :Role_Main  {
	...
	void Start(){
		skillPrefab = Resources.Load<GameObject> ("TeXiao/yangjianSkill");
	...
	{
		GameObject tmp= Instantiate (skillPrefab, transform.position + transform.forward*5, transform.rotation) as GameObject;
		if (tmp != null) {
			tmp.GetComponent<YangJianSkillControl> ().camp = this.roleCamp;
			tmp.GetComponent<YangJianSkillControl> ().owner = this;
		}
